...
//Define this macro to use x11 backend
...
int main(){
    ...
    for (int y = 0; y < PICOFB_HEIGHT; ++y) for (int x = 0; x < PICOFB_WIDTH; ++x) PICOFB_set_pixel(&picofb_window, x, y, PICOFB_pixel_rgb((x * 255) / PICOFB_WIDTH, (y * 255) / PICOFB_HEIGHT, ((x + y) * 255) / (PICOFB_WIDTH + PICOFB_HEIGHT)));

    //loop until quit
    while (!picofb_window.quit) {
        //modify frame buffer during main loop
        //set a pixel to a rgb color
        PICOFB_set_pixel(&picofb_window, PICOFB_WIDTH/2, PICOFB_HEIGHT/2, PICOFB_pixel_rgb(255, 255, 255));
